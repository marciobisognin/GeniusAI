import { slugify } from "./orgChart.js";
...
/**
 * Persistência dos squads criados pelo usuário.
 *
 * No `so-ia` isso era `window.localStorage`, que não existe fora do navegador
 * — e o próprio código já devolvia lista vazia quando `window` era indefinido.
 * Aqui a dependência vira uma porta injetável, com **o mesmo comportamento
 * padrão** (nada persiste); um host que queira persistir chama
 * `setSquadStore`.
 */
export interface SquadStore {
  read(): SquadTemplate[];
  write(created: SquadTemplate[]): void;
}

/** Padrão: não persiste — equivalente ao caminho `typeof window === "undefined"`. */
const ephemeralStore: SquadStore = {
  read: () => [],
  write: () => {},
};

let store: SquadStore = ephemeralStore;

/** Troca a persistência dos squads criados (ex.: arquivo, banco, localStorage). */
export function setSquadStore(next: SquadStore | null): void {
  store = next ?? ephemeralStore;
}

/** Store em memória, útil para hosts de processo longo e para testes. */
export function createMemorySquadStore(initial: SquadTemplate[] = []): SquadStore {
  let created = [...initial];
  return {
    read: () => [...created],
    write: (next) => {
