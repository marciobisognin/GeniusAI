...
export const ActionSchema = z.discriminatedUnion("tool", [
  ...
  z.object({ tool: z.literal("retreat_army"), args: z.object({ armyId: z.string().min(1).max(80) }) }),
  z.object({ tool: z.literal("recruit"), args: z.object({ cityId: z.string().min(1).max(80) }) }),
