...
export function buildSystemPrompt(persona: string, civId: CivId): string {
  return [
    ...
    `- attack: { armyId, x, y } — alvo adjacente; exige estar em guerra. Vencer sobre uma cidade a ocupa (produz para você).`,
    `- retreat_army: { armyId } — recua um exército instantaneamente para sua cidade própria mais próxima, sem combate.`,
